...
def build_saint_dataloader(feats, graph, dataset_name, online=False, **kwargs):
    ...
    num_subg_sampler = 1000 # the max num of subgraphs
    num_subg = 50 # the max times a node be sampled
    full = True
    batch_size_sampler = 50 # batch_size of nodes

    num_roots = 10 # max(num_nodes // 100, 2000)
...
def get_saint_subgraphs(graph, dataset_name, online=False, num_roots=100, length=4):
    num_nodes = graph.num_nodes()
    num_subg_sampler = 1000000 # the max num of subgraphs
    num_subg = 50 # the max times a node be sampled
    full = True
    batch_size_sampler = 200 # batch_size of nodes

    # num_roots = 100 # max(num_nodes // 100, 2000)
    # num_roots = 2000 # starting node for each multiple random walk
    # length = 4 # 4
    num_workers = 4
...
if __name__ == "__main__":
    ...
    # loader = build_saint_dataloader(None, g, "ogbn-arxiv")
    # for tmp_data in loader:
    #     feat, graph = tmp_data
    #     print(feat)
    #     print(graph)
